...
def simple_backtest(data: pd.DataFrame) -> pd.DataFrame:
    """Simple backtest: Buy when price > SMA20, sell when below"""
    df = data.copy()
    ...
    if st.session_state.data is not None:
        ...
        if st.button("Run Backtest"):
            with st.spinner("Running backtest..."):
                try:
                    st.session_state.results = simple_backtest(data)
        ...
        if st.session_state.results is not None:
            ...
            # Total returns
            total_strategy_return = (results_df['cumulative_returns'].iloc[-1] - 1) if 'cumulative_returns' in results_df.columns else 0
            total_benchmark_return = (results_df['benchmark_returns'].iloc[-1] - 1) if 'benchmark_returns' in results_df.columns else 0
            ...
            col1.metric("Total Strategy Return", f"{total_strategy_return:.2%}")
            col2.metric("Total Benchmark Return", f"{total_benchmark_return:.2%}")
            col3.metric("Win Rate", f"{win_rate:.2%}")
            
            # Plot equity curve
            if 'cumulative_returns' in results_df.columns:
                ...
                if 'benchmark_returns' in results_df.columns:
                    fig2.add_trace(go.Scatter(
                        x=results_df.index, 
                        y=results_df['benchmark_returns'], 
                        name='Benchmark', 
                        line=dict(width=2)
                    ))
